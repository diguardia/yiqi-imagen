# YiQi Design System — Guía maestra v1.2.8.39
...
Botón **borderless** de ícono solo (`.btn-icon`), para cualquier acción que se identifique por su ícono: cerrar un drawer, marcar favorito, bajar un Excel al lado del nombre de un reporte. Canónico en `styles.css`. `.close-btn` es **alias** del mismo objeto, conservado por compatibilidad; reemplaza las copias locales `.ds-nav-x`, `.sc-drawer-x`, `.mobile-nav-close`.
...
.btn-icon:hover         { color: var(--text); background: var(--bg-soft); }
.btn-icon:focus-visible { outline: 2px solid var(--cyan-night); outline-offset: 2px; }

.btn-icon-sm     { width: 32px; height: 32px; }   /* filas de lista y de tabla */
.btn-icon-lg     { width: 44px; height: 44px; }   /* superficies táctiles */
.btn-icon-danger { color: var(--red); }
```

- **No hereda de `.btn`**: `.btn` es padding 10/16 y relleno `color-mix`; esto es una caja cuadrada sin relleno. El prefijo es de familia, no de herencia.
- Siempre con **nombre accesible**: `aria-label` que diga qué hace y sobre qué (`"Descargar Excel — Ventas por sucursal"`), no «Descargar» a secas.
- Dentro de una fila clickeable tiene que ser un `<button>` real con `stopPropagation`, nunca un `<a>` anidado en otro `<a>`.
...
*YiQi ERP · Design System v1.2.8.39 · Última actualización: 11/09/2026*
