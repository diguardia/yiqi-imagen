...
(function (global) {
  ...
  var _TC_ICONS =
    ...
    /* "sistema" era un monitor: decia "computadora", no "sigue al tema del
       sistema, puede salir claro u oscuro". Va el circulo mitad lleno, el
       glifo estandar de automatico/contraste, que a 13px se lee. Sol y luna
       partidos se probaron y se descartaron: dos siluetas superpuestas a
       13px se emborronan. El relleno de la mitad ES el mensaje: unica
       excepcion a la familia de trazo. Necesita .tc-fill en styles.css
       porque el DS declara fill:none en el svg y el padre le gana al
       atributo del hijo. */
    '<svg class="tc-system" viewBox="0 0 24 24" width="13" height="13" fill="none" stroke="currentColor" stroke-width="1.9"><circle cx="12" cy="12" r="9"/><path class="tc-fill" d="M12 3a9 9 0 0 1 0 18Z"/></svg>' +
  ...
  /* ══════════════════════════════════════════════════════════
     ...
     Modos de data-ds-copy:
       "link"   → la URL de este bloque, con el ancla del id mas cercano
                  hacia arriba. Para compartir una seccion o una fila.
  ...
  function _copySource(btn) {
    ...
    /* "link" -> URL absoluta de este bloque, para compartirlo. El ancla sale
       del id mas cercano hacia arriba, asi que el mismo boton sirve en una
       seccion, en una subseccion o en una fila de tabla sin configurar nada.
       Sin id no copia la URL pelada: no copia. Un link que cae al tope de la
       pagina es peor que ningun link, porque el que lo recibe no se entera. */
    if (mode === 'link') {
      var anc = btn.closest('[id]');
      if (!anc || !anc.id) return '';
      return location.origin + location.pathname + location.search + '#' + anc.id;
    }
